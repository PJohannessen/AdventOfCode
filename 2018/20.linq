<Query Kind="Program">
  <Reference Relative="Dijkstra.NET.dll">D:\dev\AdventOfCode\2018\Dijkstra.NET.dll</Reference>
...
void Main()
{
    ...
    int overThreshold = 0;
    foreach (var node in nodes.Values.Where(n => n != 0))
    {
        ShortestPathResult currentShortestPath = graph.Dijkstra(0, node);
        if (currentShortestPath.Distance > longestShortestPath) longestShortestPath = currentShortestPath.Distance;
        if (currentShortestPath.Distance >= threshold) overThreshold++;
    }

    $"P1: {longestShortestPath}".Dump();
    $"P2: {overThreshold}".Dump();
}

void ProcessSlice(int x, int y)
{
    int startingX = x;
    int startingY = y;
    int currentX = x;
    ...
    {
        uint currentNode = nodes[new Tuple<int, int>(currentX, currentY)];
        char c = instructions[index];
        if ("NSWE".Contains(c))
        {
            if (c == 'W') currentX -= 1;
            else if (c == 'E') currentX += 1;
            else if (c == 'N') currentY -= 1;
            else if (c == 'S') currentY += 1;
            
            uint nextNode = 0;
            if (nodes.ContainsKey(new Tuple<int, int>(currentX, currentY)))
                nextNode = nodes[new Tuple<int, int>(currentX, currentY)];
            else
            {
                nextNode = nodeCounter;
                nodes.Add(new Tuple<int, int>(currentX, currentY), nodeCounter);
                graph.AddNode(nodeCounter);
                nodeCounter++;
            }
            
            graph.Connect(currentNode, nextNode, 1, "");
            index++;
        }
        else if (c == '|')
        {
            index++;
            ProcessSlice(startingX, startingY);
            return;
        }
        else if (c == '(')
        {
            index++;
            ProcessSlice(currentX, currentY);
        }
        else if (c == ')')
        {
            index++;
