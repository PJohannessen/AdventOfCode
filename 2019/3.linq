...
void Main()
{
    ...
    foreach (var instruction in wire1)
    {
        ...
        for (int i = 0; i < stepsToTake; i++)
        {
            ...
            var position = (x, y);
            if (!visited.ContainsKey(position)) visited.Add(position, (false, counter));
        }
    }
    ...
    foreach (var instruction in wire2)
    {
        ...
        for (int i = 0; i < stepsToTake; i++)
        {
            ...
            var position = (x, y);
            if (visited.ContainsKey(position)) visited[position] = (true, visited[position].StepCount + counter);
        }
    }
        
    int shortestDistance = visited.Where(p => p.Value.VisitedTwice).Min(p => Math.Abs(p.Key.x) + Math.Abs(p.Key.y));
    int quickestIntersection =  visited.Where(p => p.Value.VisitedTwice).Min(p => p.Value.StepCount);
    
    $"P1: {shortestDistance}".Dump();
    $"P2: {quickestIntersection}".Dump();
}