...
void Main()
{
    ...
    while (instructions.Count > 0)
    {
        for (int i = instructions.Count - 1; i >= 0; i--)
        {
            ...
            {
                if (wires.ContainsKey(parts[1])) wires.Add(parts[2], (ushort)(~wires[parts[1]]));
            }
            else
            {
                if (IsNumber(parts[0])) wires.Add(parts[1], Convert.ToUInt16(parts[0]));
                if (wires.ContainsKey(parts[0])) wires.Add(parts[1], wires[parts[0]]);
            }
            if (wires.Count > wiresDone) instructions.RemoveAt(i);
        }
    }
    wires.Dump();
}
